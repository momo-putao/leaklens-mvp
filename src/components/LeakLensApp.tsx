...
import { DocumentReview, RewriteMode, RiskTag, ReviewScenario } from "@/lib/types";
...
export function LeakLensApp() {
  ...
  const stats = useMemo(() => {
    ...
    return { high, medium, training, topTag };
  }, [reviews]);
          ...
          <header className="rounded-lg border border-line bg-white/92 p-5 shadow-soft backdrop-blur">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                ...
                <MetricCard label="高频标签" value={stats.topTag} icon={Gauge} tone="bg-emerald-50 text-emerald-700" />
              </div>
            </div>
          </header>
